...
public class CheckX509Certificate {
    public static void main(String[] args) throws Exception {
        try {
            ...
            X509Certificate cacert = (X509Certificate) certificateFactory.generateCertificate(inputStream1);
            //System.out.println(cacert); // DEBUG

            // check if certificates are expired
            controllerCertificate.checkValidity(new Date());
            cacert.checkValidity(new Date());

            controllerCertificate.verify(cacert.getPublicKey());
