...
LOW_WATERMARK = 3 -- device count for when to decommission a sub-controller
...
net:set_callback(function(my_net, devices) -- set_callback is called every 5000ms
    ...
    if #our_devices >= HIGH_WATERMARK then -- sub controller needed
    ...
    else if #our_devices <= LOW_WATERMARK then -- no sub controller needed anymore, get nodes back
        ...
        for i, device in ipairs(devices) do
            while #our_devices <= LOW_WATERMARK do
                if device.is_sub_controller == true do
